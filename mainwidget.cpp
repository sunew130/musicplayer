...
/*
  ...
  //  args.append(tr(" -wid "));
   // args.append(QString::number(ui->label->winId()));
    args << "-slave";
    args << "-quiet";
    args << m_movieFile;
    qDebug()<<args;
   ...
   /*
    ...
    str <<"-slave -quiet -fps 20 ";
    str.append(m_movieFile);
    str.append(tr(" -wid "));
    str.append(QString::number(ui->label->winId()));
*/
    m_mplayerProcess->setProcessChannelMode(QProcess::MergedChannels);
    m_mplayerProcess->start(MPLAYER_PATH,str);//MPLAYER_PATH,
...
{
    QString filename("Media Files(*.mp4 *.avi)");
    stopMPlayer();

    QFileDialog *fileDialog = new QFileDialog(this);
    fileDialog->setWindowTitle(filename);
    fileDialog->setDirectory("/usr/local/adsdemo/ads");
    //fileDialog->setFilter(//(QDir::Filter);
